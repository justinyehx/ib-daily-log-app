...
export default async function AdminViewPage({ params, searchParams }: AdminViewPageProps) {
  const { storeSlug } = await params;
  const resolvedSearchParams = searchParams ? await searchParams : undefined;
  const session = await getCurrentSession();

  if (!session.isAuthenticated) redirect("/login");
  if (session.role !== "ADMIN") redirect(`/${session.storeSlug}/dashboard`);
  ...
  const hasSpecificFilters = Boolean(
    admin.filters.store || admin.filters.pricePoint ||
    admin.filters.visitType || admin.filters.appointmentType ||
          ...
          <div className="table-wrap compact-table">
            <table className="data-table admin-store-table">
              <thead>
                <tr>
                  {[
                    ["storeLabel", "Store"], ["guestsSeen", "Guests Seen"],
                    ["bridesSeen", "Brides Seen"], ["bridesSold", "Brides Sold"],
                    ["closeRate", "Close Rate"], ["cbRate", "CB Rate"],
                    ["addOnRate", "Add-on Rate"], ["averageDuration", "Avg Duration"]
                  ].map(([key, label]) => {
                    const isActive = admin.storeSort.key === key;
                    const nextDirection = isActive && admin.storeSort.direction === "desc" ? "asc" : "desc";
                    return (
                      <th key={key}>
                        <Link className={`sort-button ${isActive ? "active" : ""}`} href={buildQuery(resolvedSearchParams, { storeSortKey: key, storeSortDirection: nextDirection })}>
                          <span className="sort-button-label">{label}</span>
                          {isActive ? <span className="sort-button-arrow">{admin.storeSort.direction === "asc" ? "↑" : "↓"}</span> : null}
                        </Link>
                      </th>
                    );
                  })}
              ...
              <tbody>
                {admin.stores.map((store) => (
                  <tr key={store.storeId}>
                    ...
                    <td>{store.guestsSeen}</td><td>{store.bridesSeen}</td>
                    <td>{store.bridesSold}</td><td>{formatPercent(store.closeRate)}</td>
                    <td>{formatPercent(store.cbRate)}</td><td>{formatPercent(store.addOnRate)}</td>
          ...
          <div className="table-wrap compact-table">
            <table className="data-table admin-leaderboard-table">
              <thead>
                <tr>
                  {[
                    ["name", "Stylist"], ["storeLabel", "Store"],
                    ["guestsSeen", "Guests Seen"], ["bridesSeen", "Brides Seen"],
                    ["bridesSold", "Brides Sold"], ["closeRate", "Close Rate"],
                    ["averageDuration", "Avg Duration"], ["cbRate", "CB Rate"],
                    ["addOnRate", "Add-on Rate"]
                  ].map(([key, label]) => {
                    const isActive = admin.leaderboardSort.key === key;
                    const nextDirection = isActive && admin.leaderboardSort.direction === "desc" ? "asc" : "desc";
                    return (
                      <th key={key}>
                        <Link className={`sort-button ${isActive ? "active" : ""}`} href={buildQuery(resolvedSearchParams, { sortKey: key, sortDirection: nextDirection })}>
                          <span className="sort-button-label">{label}</span>
                          {isActive ? <span className="sort-button-arrow">{admin.leaderboardSort.direction === "asc" ? "↑" : "↓"}</span> : null}
                        </Link>
                      </th>
                    );
                  })}
              ...
              <tbody>
                {admin.leaderboard.map((row) => (
                  <tr key={`${row.storeLabel}-${row.name}`}>
                    <td>{formatStaffDisplayName(row.name)}</td><td>{row.storeLabel}</td>
                    <td>{row.guestsSeen}</td><td>{row.bridesSeen}</td>
                    <td>{row.bridesSold}</td><td>{formatPercent(row.closeRate)}</td>
                    <td>{formatMinutes(row.averageDuration)}</td>
                    <td>{formatPercent(row.cbRate)}</td><td>{formatPercent(row.addOnRate)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
