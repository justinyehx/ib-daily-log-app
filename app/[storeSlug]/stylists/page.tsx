...
export default async function StylistsPage({ params, searchParams }: StylistsPageProps) {
  ...
  const stylists = await getStylistsData(storeSlug, forcedSearchParams, timezone);

  if (!stylists) return null;
          ...
          <div className="stylist-grid">
            {stylists.leaderboard.length ? (
              stylists.leaderboard
                .filter((entry) =>
                  entry.appointmentsCount > 0 || entry.bridesSeen > 0 ||
                  entry.bridesSold > 0 || entry.averageDuration > 0 ||
                  entry.cbRate > 0 || entry.addOnRate > 0
                )
                ...
                .map((entry) => (
                  ...
                  >
                    ...
                    <div className="score-grid">
                      <div><span className="summary-label">Appointments</span><strong className="score-value">{entry.appointmentsCount}</strong></div>
                      <div><span className="summary-label">Brides Seen</span><strong className="score-value">{entry.bridesSeen}</strong></div>
                      <div><span className="summary-label">Brides Sold</span><strong className="score-value">{entry.bridesSold}</strong></div>
                      <div><span className="summary-label">Avg Time</span><strong className="score-value">{formatMinutes(entry.averageDuration)}</strong></div>
                      <div><span className="summary-label">CB Rate</span><strong className="score-value">{formatPercent(entry.cbRate)}</strong></div>
                      <div><span className="summary-label">Add-ons</span><strong className="score-value">{formatPercent(entry.addOnRate)}</strong></div>
                    </div>
                  </Link>
                ))
            ) : (
                    ...
                    <div className="stack-list">
                      {stylists.detailReasonTallies.map((item) => (
                        <div className="stack-item" key={item.label}>
                          ...
                          <div className="bar">
                            <span style={{ width: `${(item.value / (stylists.detailReasonTallies[0]?.value || 1)) * 100}%` }} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="stylist-breakdown-empty">No &quot;did not purchase&quot; reasons in this reporting window.</div>
                  )}
                </section>
              </div>
                ...
                <table className="data-table stylist-detail-table">
                  <thead>
                    <tr>
                      ...
                      <th>Date</th><th>Guest</th><th>Appt</th><th>Visit</th>
                      <th>Location</th><th>Time In</th><th>Time Out</th>
                      <th>Duration</th><th>Purchased</th><th>Other Sale</th>
                  ...
                  <tbody>
                    {stylists.appointmentRows.map((row) => (
                      <tr key={row.id}>
                        ...
                        <td>{row.date}</td><td>{row.guestName}</td>
                        <td>{row.appointmentType}</td><td>{row.visitType}</td>
                        <td>{row.location}</td><td>{row.timeIn}</td>
                        <td>{row.timeOut}</td><td>{row.duration}</td>
                        <td>{row.purchased}</td><td>{row.otherSale}</td>
